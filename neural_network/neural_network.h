    ...
    void fit(int epochs, double** X, double** y, int m)
    {
        ...
        for (int epoch = 0; epoch < epochs; epoch++) {
            for (int i = 0; i < m; i++) {
                ...
                this->hidden_layers[this->n_layers - 1].backward(
                    this->output_layer.access_neurons_curr(), 
                    this->output_layer.access_W(), 
                    this->output_layer.access_b(), 
                    this->output_layer.access_delta()
                );
                for (int l = this->n_layers - 2; l > -1; l--) {
                    this->hidden_layers[l].backward(
                        this->hidden_layers[l + 1].access_neurons_curr(), 
                        this->hidden_layers[l + 1].access_W(), 
                        this->hidden_layers[l + 1].access_b(), 
                        this->hidden_layers[l + 1].access_delta()
                    );
                }
                cout << endl;
            }
